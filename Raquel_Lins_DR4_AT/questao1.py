...
def exportar_dados(dataframe, nome_do_arquivo):
    """
    Exporta os dados de um DataFrame para CSV, JSON e Excel.
    ...
    try:
        if dataframe is not None:
            ...
            #force_ascii false para nao termos problemas com caracteres.
            dataframe.to_json(f"Dados_Jogos/{nome_do_arquivo}.json", orient='records', force_ascii=False)
            dataframe.to_excel(f"Dados_Jogos/{nome_do_arquivo}.xlsx")
            print(f">>DADOS EXPORTADOS<< : {nome_do_arquivo}.csv, {nome_do_arquivo}.json e {nome_do_arquivo}.xlsx")
    except Exception as e:
